...
3. **Publikus foglalásmentés.** A jelenlegi validációra épül, de tranzakción belül újraellenőrzi a rendelkezésre állást és idempotency kulcsot használ. Elfogadás: párhuzamos ütközésből legfeljebb egy foglalás jön létre, státuszelőzmény készül. Lásd [publikus flow](03_PUBLIC_BOOKING_FLOW.md).
...
| ADR-007 | IMPLEMENTED | Repository interfész a read oldalon | A domain/application réteg nem függ közvetlenül PDO-tól; write repository még PLANNED. |
| ADR-008 | PLANNED | SMTP transport és e-mail outbox/log | Közvetlen `mail()` tiltott; szolgáltató és library még DECISION REQUIRED. |
...
| ADR-012 | PLANNED | Megváltoztathatatlan ár-pillanatkép | Későbbi árszabály-változás nem írja át a korábbi ajánlat/foglalás árát. |
...
7. **DECISION REQUIRED:** `pending` foglalás tartási ideje, illetve blokkolja-e valaha a naptárt.
