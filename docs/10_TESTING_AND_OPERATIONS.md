...
|---|---|---|---|
| Unit | tiszta domain szabályok: fél-nyitott intervallum, napállapot, validáció | **IMPLEMENTED** availability területen | **PLANNED:** pricing, státuszállapotgép, 2FA, e-mail és iCal szabályok |
...
| Feature | controller és use case együtt, kontrollált függőségekkel | **IMPLEMENTED** availability controller alapesetekre; router/front controller dispatch nincs lefedve | **PLANNED:** valódi HTTP dispatch, booking validation/write, admin auth, pricing, iCal admin műveletek |
...
**PLANNED:** tesztelendő az éjszakaszám, személy- és gyermekkor-sáv, szezon/hétvége prioritás, fix díj, IFA, kedvezmény, HUF-kerekítés, admin override és megváltoztathatatlan snapshot. Minden elfogadott [pricing szabályhoz](05_PRICING.md) legalább határérték-, kombinációs és regressziós példa szükséges.

### Párhuzamos foglalás és idempotencia

**PLANNED:** két külön adatbázis-kapcsolat szinkronizáltan próbálja ugyanazt az időszakot menteni. Elvárt: legfeljebb egy siker, a másik domain-konfliktus; nincs részleges booking/guest/history rekord. Azonos idempotenciakulcs ismétlése ugyanazt az eredményt adja, eltérő payload esetén elutasítandó.

### Státuszváltás

**PLANNED:** minden engedélyezett és tiltott átmenet, konkurens adminmódosítás, status history és audit tesztelendő. Tiltott átmenet nem módosíthat bookingot és nem küldhet e-mailt.

### Admin és 2FA

**PLANNED:** helyes/hibás jelszó, ismeretlen felhasználó időzítésének kiegyenlítése, kód hash, 10 perces tervezett lejárat, maximum 5 próbálkozás, resend limit, egyszer használhatóság, session rotation, logout/revocation, CSRF, rate limit és lockout. Részletek: [admin és hitelesítés](04_ADMIN_AND_AUTHENTICATION.md).
