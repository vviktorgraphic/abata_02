...
$url = trim(getenv('PRIVACY_POLICY_URL') ?: '');
$version = trim(getenv('PRIVACY_POLICY_VERSION') ?: '');
...
$developmentHttp = in_array($environment, ['development', 'local', 'testing'], true)
    && $scheme === 'http' && filter_var($url, FILTER_VALIDATE_URL) !== false;

if (!$relative && !$https && !$developmentHttp) {
    throw new RuntimeException('PRIVACY_POLICY_URL must be relative or HTTPS; HTTP is allowed only in development.');
}
